...
## 4. Registro local en servidor (opcional)

`GET` y `POST` sobre `/api/miinfo` leen o guardan **un** objeto `datos` (nombre, apellido, teléfono, email) en el backend (sin gas), al margen de la ficha on-chain. Cada `POST` reemplaza el registro. La app de emergencia **no** depende de esto.
...
- JSON: <http://localhost:3000/openapi.json> (importar en Postman, Insomnia, o generar cliente).

---

## 7. QR de emergencia (propuesta)
